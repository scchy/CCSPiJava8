 ...
 * @author Scc_hy
 * @version 1.0.0
 * @ClassName Governor.java
 * @Description 按年龄和经度对洲长进行聚类
 * @createTime 2023年03月26日 18:58:00
 */
public class Governor extends DataPoint{
    ...
    public static void main(String[] args){
        ...
        KMeans<Governor> kms = new KMeans<>(2, governors);
        List<KMeans<Governor>.Cluster> govCluster = kms.run(100);
        for (int clusterIndex = 0; clusterIndex < govCluster.size(); clusterIndex++) {
            System.out.printf("Cluster %d: %s%n", clusterIndex, govCluster.get(clusterIndex).points);
        }
    }
}
